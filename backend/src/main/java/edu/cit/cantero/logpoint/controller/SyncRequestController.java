...
public class SyncRequestController {
    ...
    // guardId -> true when a live sync relationship is active
    ...
    // GET /api/sync/logs/{guardId} — initial collect after acceptance
    ...
    // GET /api/sync/live/{guardId} — real-time refresh, no status check needed
    // Admin can call this at any time as long as there's an active sync
    ...
    // POST /api/sync/activate/{guardId} — marks a guard as actively synced (called after successful collect)
    ...
    // POST /api/sync/deactivate/{guardId} — admin stops live sync
    @PostMapping("/deactivate/{guardId}")
    public ResponseEntity<?> deactivateSync(@PathVariable Long guardId, HttpSession session) {
        UserDTO user = (UserDTO) session.getAttribute("user");
        if (user == null) return ResponseEntity.status(401).body("Not authenticated");
        if (!"office administrator".equalsIgnoreCase(user.getRole()))
            return ResponseEntity.status(403).body("Forbidden");
        ...
        return ResponseEntity.ok(Map.of("message", "Sync deactivated"));
    }
}